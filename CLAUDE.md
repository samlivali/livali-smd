...
- **Phase 1 (user, external) — done.** Meta Developer App created (type Business), Facebook Login for Business added, OAuth redirect URI set to `https://adccekhhqeprxbwcentk.supabase.co/functions/v1/meta-oauth-callback`. App ID `1465456148962040`. App Secret is in the `META_APP_SECRET` Supabase Edge Function secret (never in chat, never in the repo).
- **Phase 2 (me) — OAuth half done, publishing half not started.**
  - Supabase project is now `supabase init`'d and linked (`supabase link --project-ref adccekhhqeprxbwcentk`) inside this repo, so the CLI reads `supabase/` here. CLI auth is per-machine (`supabase login`) — not something I can do myself, needs a real browser; use `npx supabase <cmd>` if `supabase` isn't on PATH in a given terminal.
  - `social_accounts` table (migration `supabase/migrations/20260725060543_create_social_accounts.sql`): holds real OAuth Page/IG tokens per client. **RLS enabled with zero anon/authenticated policies** — deliberately not readable via the anon key like `app_state` is, since `page_access_token` is a real credential. Only `service_role` (used inside Edge Functions) can touch it. `social_accounts_public` is a view exposing just `client_id, platform, page_name, connected_at` and is anon-readable, for the frontend to show connection status without ever seeing the token.
  - `supabase/functions/meta-oauth-start`: `GET ?clientId=xxx` → signs a `state` (HMAC-SHA256 using `META_APP_SECRET`, embeds `clientId` + nonce + timestamp, 10-min expiry) and 302-redirects into Meta's OAuth dialog. Deployed with `--no-verify-jwt` since it's a plain link click, no Supabase auth header.
  - `supabase/functions/meta-oauth-callback`: verifies `state`, exchanges `code` → short-lived → long-lived user token, calls `/me/accounts` for Pages (**picks the first Page returned** — pilot simplification, revisit if a client ever needs a Page picker), pulls `instagram_business_account` off that Page if linked, upserts one or two rows into `social_accounts` via the REST API with the service-role key, then redirects back to `https://samlivali.github.io/livali-smd/?meta_connect=success|error&meta_detail=...`. Also deployed `--no-verify-jwt`.
  - Shared signing/constants logic lives in `supabase/functions/_shared/meta.ts` (scopes requested: `pages_show_list, pages_read_engagement, pages_manage_posts, instagram_basic, instagram_content_publish, business_management`).
  - Both functions smoke-tested via curl (redirect shape, missing-params error path, tampered-state rejection) — not yet tested with a real Meta login end-to-end.
  - **Not started yet**: the scheduled-publishing function (cron-triggered, replaces `checkStatus()`'s random coin flip with real Graph API publish calls + status updates back into the `app_state` JSON blob). This needs care since posts live in one JSON column, not a relational table — a raw read-modify-write from a cron function risks clobbering a concurrent frontend save; likely wants a narrow Postgres function/RPC that patches a single post by id inside the JSON array rather than rewriting the whole blob.
- **Phase 3 (me) — not started.** "Connect Facebook/Instagram" UI per client (a link to `meta-oauth-start?clientId=...`, plus reading `social_accounts_public` to show connected status), wire real publishing into the existing post scheduler.
...
- Plan is to pilot with a **test Facebook Page**, not QuikDawa's real page, until the flow is proven end-to-end.
